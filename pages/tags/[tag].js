...
const Index = () => {
  ...
  return (
    <HomeLayout
      smallerDiv={false}
      title={{
        headerTitle: `${filterType} Documents on Tag: ${capTag}`,
        mainTitle: capTag,
      }}
    >
      <Meta title={`Documents on Tag: ${capTag} | devBugDocs`} />
      <div>
        <DocumentList
          documents={tagDocuments}
          setFilterType={setFilterType}
          capTitle={`${capTag} tag`}
          documentTypeName={SET_TAG_DOCUMENTS}
          _404msg={`There are no documents on ${capTag} yet`}
          dashboard={false}
