...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const doc = type !== 'draft' ? documents.filter(document => document.id === id) : userDocs.filter(document => document.id === id);
  ...
  return (
    <HomeLayout smallerDiv={true}>
      <Meta
        title={document.title ? document.title.substr(0, 1).toUpperCase() + document.title.substr(1) : 'loading...'}/>
